...
export class StatusService {
  ...
  async getPublicStatus(organizationSlug: string) {
    ...
    const [monitors, checks, activeIncidents, recentIncidents] = await Promise.all([
    ...
    const summary = this.buildSummary(monitors, checks, activeIncidents, recentIncidents, now);
  ...
  private getOverallStatus(monitors: MonitorSnapshot[]) {
    if (monitors.some((monitor) => monitor.currentStatus === MonitorStatus.DOWN)) {
      return 'DEGRADED';
    }

    if (monitors.some((monitor) => monitor.currentStatus === MonitorStatus.UNKNOWN)) {
      return 'PARTIAL';
    }
  ...
  private buildMonitorStats(monitors: MonitorSnapshot[], checks: Array<{ monitorId: number; status: MonitorStatus; responseTimeMs: number | null; checkedAt: Date }>) {
    ...
    return monitors.map((monitor) => {
      ...
      const upChecks = monitorChecks.filter((check) => check.status === MonitorStatus.UP).length;
      ...
      return {
        ...monitor,
        sla: {
          uptimePercentage: monitorChecks.length ? Number(((upChecks / monitorChecks.length) * 100).toFixed(2)) : null,
  ...
  private buildDailyHistory(checks: Array<{ status: MonitorStatus; checkedAt: Date }>) {
    ...
    return Array.from(days.entries()).map(([date, value]) => ({
      date,
      uptimePercentage: value.total ? Number(((value.up / value.total) * 100).toFixed(2)) : null,
      checks: value.total,
    }));
  }
  ...
  private buildSummary(
    ...
    recentIncidents: Array<{ status: IncidentStatus; startedAt: Date; resolvedAt: Date | null; durationSeconds: number | null }>,
    now: Date,
  ) {
    ...
    const downtimeSeconds = recentIncidents.reduce((total, incident) => {
      ...
      if (incident.status !== IncidentStatus.RESOLVED) {
        return total + Math.max(0, Math.floor((now.getTime() - incident.startedAt.getTime()) / 1000));
      }
      return total;
    }, 0);
    ...
    return {
      ...
      operationalMonitors: monitors.filter((monitor) => monitor.currentStatus === MonitorStatus.UP).length,
      degradedMonitors: monitors.filter((monitor) => monitor.currentStatus === MonitorStatus.DOWN).length,
      unknownMonitors: monitors.filter((monitor) => monitor.currentStatus === MonitorStatus.UNKNOWN).length,
      activeIncidents: activeIncidents.length,
      incidentsLast30d: recentIncidents.length,
      checksLast30d: checks.length,
      uptimeLast30d: checks.length ? Number(((upChecks / checks.length) * 100).toFixed(2)) : null,
      avgResponseTimeMs: responseTimes.length
        ? Math.round(responseTimes.reduce((total, value) => total + value, 0) / responseTimes.length)
        : null,
